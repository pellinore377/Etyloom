...
impl Fixture {
    async fn new() -> Result<Self> {
        ...
        let config = Config {
            base_url: "http://127.0.0.1:3000".parse()?,
            address: "127.0.0.1:3000".parse()?,
            database_url,
            site_root: directory.path().into(),
    ...
    async fn request(
        ...
        if signed {
            request = request.header("cookie", format!("etyloom_session={}", self.token));
        }
        if csrf {
    ...
    async fn generated(&self) -> Result<String> {
        ...
        let (status, value) = self
            .request("POST", "/api/generate", body, true, true)
            .await?;
        ...
        for _ in 0..100 {
            ...
            if current.state == "completed" {
                ...
                return current
                    .language_id
                    .ok_or_else(|| "Completed job has no language".into());
            }
            if current.state == "failed" {
                worker.abort();
                return Err(format!("Generation failed: {:?}", current.error).into());
            }
...
async fn authentication_csrf_and_project_isolation() -> Result<()> {
    ...
    assert_eq!(
        fixture
            .request("GET", "/api/projects", Value::Null, false, false)
            .await?
            .0,
...
async fn generation_translation_publication_and_export() -> Result<()> {
    ...
    let (_, value) = fixture
        .request(
            "GET",
            &format!("/api/languages/{id}/export"),
            Value::Null,
            true,
            false,
        )
    ...
    let body = serde_json::to_value(TranslationRequest {
        text: "I see the river".into(),
        reverse: false,
    })?;
    let (status, translated) = fixture
        .request(
            "POST",
            &format!("/api/languages/{id}/translate"),
            body,
            true,
            true,
        )
    ...
    assert_eq!(
        fixture
            .request(
                "POST",
                &format!("/api/languages/{id}/publish"),
                publish.clone(),
                true,
                true
            )
...
async fn package_import_checks_checksum_and_ownership() -> Result<()> {
    let fixture = Fixture::new().await?;
    let mut package = etyloom_engine::generate(Recipe {
        lexicon_size: 128,
        ..Recipe::default()
    })?;
    ...
    let (status, _) = fixture
        .request(
            "POST",
            "/api/import",
            json!({"project_id":"ours","package":package}),
            true,
            true,
        )
        .await?;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    let (_, languages) = fixture
        .request("GET", "/api/languages", Value::Null, true, false)
        .await?;
