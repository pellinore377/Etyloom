...
impl User {
    pub fn csrf(&self, headers: &HeaderMap, state: &AppState) -> Result<()> {
        ...
        if origin.is_some_and(|value| value != state.config.origin()) {
        ...
        if !bool::from(token.as_bytes().ct_eq(self.csrf.as_bytes())) {
            return Err(AppError::Forbidden);
        }
        Ok(())
    }
...
pub fn random_token() -> Result<String> {
    ...
    getrandom::getrandom(&mut bytes)
        .map_err(|e| AppError::Internal(anyhow!("Operating-system randomness failed: {e}")))?;
...
fn set_cookie(
    ...
    .parse()
    .map_err(|e| AppError::Internal(anyhow!("Cookie serialization failed: {e}")))
}

async fn current(state: &AppState, headers: &HeaderMap) -> Result<Option<User>> {
    let Some(token) = cookie(headers, "etyloom_session") else {
        return Ok(None);
    };
...
pub async fn session(
    ...
    Ok(Json(SessionInfo {
        ...
        display_name: user.as_ref().map(|u| u.name.clone()).unwrap_or_default(),
        csrf: user.as_ref().map(|u| u.csrf.clone()).unwrap_or_default(),
...
async fn client(state: &AppState) -> Result<Client> {
    let metadata = CoreProviderMetadata::discover_async(
        IssuerUrl::new(state.config.issuer.clone()).context("Invalid issuer")?,
        &state.http,
    )
    .await
    .context("Pocket ID discovery failed")?;
...
pub async fn login(State(state): State<AppState>, headers: HeaderMap) -> Result<Response> {
    if state.config.development {
        return dev_login(State(state)).await;
    }
    sqlx::query("DELETE FROM logins WHERE expires_at<=unixepoch()")
        .execute(&state.db)
        .await?;
    if let Some(old) = cookie(&headers, "etyloom_login") {
        sqlx::query("DELETE FROM logins WHERE token_hash=?")
            .bind(digest(old))
            .execute(&state.db)
            .await?;
    }
    let (count,): (i64,) = sqlx::query_as("SELECT count(*) FROM logins")
        .fetch_one(&state.db)
        .await?;
    if count >= 1000 {
    ...
    response.headers_mut().append(
        header::SET_COOKIE,
        set_cookie(&state, "etyloom_login", &token, 600)?,
    );
    Ok(response)
}
...
pub async fn callback(
    ...
    if !bool::from(supplied.as_bytes().ct_eq(expected.as_bytes())) || query.error.is_some() {
    ...
    let tokens = client
        ...
        .context("Token endpoint unavailable")?
    ...
    let id = tokens
        .id_token()
        .ok_or_else(|| AppError::Internal(anyhow!("Identity provider returned no ID token")))?;
    let claims = id
        .claims(&client.id_token_verifier(), &Nonce::new(nonce))
        .context("OIDC token validation failed")?;
    let name = claims
        .preferred_username()
        .map(|n| n.as_str())
        .unwrap_or("Etyloom member");
    ...
    response.headers_mut().append(
        header::SET_COOKIE,
        set_cookie(&state, "etyloom_login", "", 0)?,
    );
    Ok(response)
}

pub async fn dev_login(State(state): State<AppState>) -> Result<Response> {
    if !state.config.development || !cfg!(debug_assertions) {
        return Err(AppError::NotFound);
    }
...
async fn establish(state: &AppState, user: i64, headers: &HeaderMap) -> Result<Response> {
    ...
    if let Some(old) = cookie(headers, "etyloom_session") {
    ...
    response.headers_mut().append(
        header::SET_COOKIE,
        set_cookie(state, "etyloom_session", &token, 1_209_600)?,
    );
    Ok(response)
}
...
pub async fn logout(
    ...
    response.headers_mut().append(
        header::SET_COOKIE,
        set_cookie(&state, "etyloom_session", "", 0)?,
    );
    Ok(response)
}
...
mod tests {
    ...
    fn duplicate_session_cookie_is_rejected() {
        ...
        headers.insert(
            header::COOKIE,
            header::HeaderValue::from_static("etyloom_session=a; etyloom_session=b"),
        );
