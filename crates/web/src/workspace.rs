            ...
            <div class="creation-layout">
                <form class="creation-form" on:submit=submit>
                    <fieldset disabled=move || busy.get() || job.get().is_some()>
                        ...
                        <section class="form-section"><div class="section-index">"03"</div><div><h2>"Leave room for a past."</h2><div class="form-grid"><label>"Lexical entries"<select name="size" on:change=move |event| size.set(event_target_value(&event))><option value="512">"512 — a small working language"</option><option value="128">"128 — a compact exploration"</option><option value="2048">"2,048 — an expanded vocabulary"</option><option value="4096">"4,096 — a broad collection"</option><option value="5000">"5,000 — maximum vocabulary"</option></select></label><label>"Historical developments"<select on:change=move |event| depth.set(event_target_value(&event))><option value="4">"4 periods — an evolving vocabulary"</option><option value="1">"1 period — a recent beginning"</option><option value="3">"3 periods — sound changes leave a trace"</option><option value="6">"6 periods — a longer recorded history"</option><option value="8">"8 periods — the longest current window"</option></select></label></div><p class="field-help">"Larger vocabularies include transparent compounds. Complexity comes from interacting rules, not an entry count."</p></div></section>
                        ...
                        <details class="advanced"><summary>"Advanced choices & imported recipe"</summary><div class="advanced-content"><label>"Constituent order"<select on:change=move |event| order.set(event_target_value(&event))><option value="auto">"Choose automatically"</option>{["svo","sov","vso","vos","ovs","osv"].into_iter().map(|value| view! { <option value=value>{value.to_uppercase()}</option> }).collect_view()}</select></label><label>"Speech community"<input maxlength="240" on:input=move |event| community.set(event_target_value(&event))/></label><label>"World notes (reference only)"<textarea rows="4" maxlength="8000" on:input=move |event| notes.set(event_target_value(&event))></textarea></label><p class="field-help">"Notes are preserved, not automatically interpreted or used to invent cultural rules."</p><label>"Paste an exported recipe"<textarea class="mono" rows="6" placeholder="Complete recipe JSON overrides the choices above" prop:value=move || recipe_json.get() on:input=move |event| recipe_json.set(event_target_value(&event))></textarea></label><button class="text-button" type="button" on:click=move |_| {
                            let result = (|| -> etyloom_core::Result<String> { let mut recipe: Recipe = serde_json::from_str(&recipe_json.get_untracked())?; recipe.upgrade()?; Ok(serde_json::to_string_pretty(&recipe)?) })();
                            match result { Ok(value) => { recipe_json.set(value); error.set(None); }, Err(message) => error.set(Some(message.to_string())) }
                        }>"Upgrade imported recipe to current generator"</button><p class="field-help">"An imported recipe otherwise uses its original generator. Upgrading changes the result, not your saved language."</p></div></details>
