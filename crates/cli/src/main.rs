...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.first().map(String::as_str) {
        Some("generate") => {
            let recipe = if let Some(path) = args.get(1) {
        ...
        Some("validate") => {
            ...
            println!("Valid revision {}", package.revision);
        }
        Some("translate") | Some("analyze") => {
            let package = load(args.get(1))?;
            let text = args
                .get(2)
                .ok_or("Provide a sentence as one quoted argument")?;
            let translated = Runtime::new(&package)?
                .translate(text, args.first().is_some_and(|s| s == "analyze"))?;
        ...
        Some("bench") => {
            ...
            let runs = args
                ...
                .unwrap_or(20);
            if runs == 0 || runs > 1000 {
                return Err("Choose 1–1,000 benchmark runs".into());
            }
            let mut elapsed = Vec::new();
            for i in 0..runs {
                let recipe = Recipe {
                    seed: format!("benchmark-{i}"),
                    lexicon_size: entries,
                    ..Recipe::default()
                };
                ...
                elapsed.push(millis);
                eprintln!(
                    "run={i} entries={} elapsed_ms={millis} revision={}",
            ...
            println!(
                "entries={entries} runs={runs} p95_ms={}",
                elapsed.get(index).ok_or("Missing benchmark result")?
            );
        }
        _ => {
            writeln!(
