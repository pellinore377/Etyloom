...
pub fn examples(package: &Package, count: usize) -> Result<Vec<Example>> {
    if count > 100_000 {
        return Err(Error::Invalid(
            "Corpus limit is 100,000 scenes per export".into(),
        ));
    }
    let runtime = Runtime::new(package)?;
    let verbs: Vec<_> = package
        .lexicon
        .iter()
        .filter(|e| e.category == Category::Verb)
        .collect();
    let nouns: Vec<_> = package
        .lexicon
        .iter()
        .filter(|e| e.category == Category::Noun && matches!(e.origin, Origin::Root))
        .collect();
    let adjectives: Vec<_> = package
        .lexicon
        .iter()
        .filter(|e| e.category == Category::Adjective)
        .collect();
    ...
    for i in 0..count {
        ...
        let subject = if i % 3 == 0 {
            Entity::Noun {
                id: rng.pick(&nouns)?.id.clone(),
                plural: rng.below(2)? == 0,
                adjective: None,
            }
        } else {
            Entity::Pronoun {
                person: (rng.below(3)? + 1) as u8,
                plural: rng.below(2)? == 0,
            }
        };
        let object = if verb.transitive {
            Some(Entity::Noun {
                ...
                plural: rng.below(2)? == 0,
                adjective: if i % 4 == 0 {
                    Some(rng.pick(&adjectives)?.id.clone())
                } else {
                    None
                },
            })
        ...
        let tense = if matches!(i % 8, 2 | 3 | 6 | 7) {
            Tense::Present
        } else {
            rng.pick(&[Tense::Present, Tense::Past, Tense::Future])?
        };
        let event = Meaning::Event {
        ...
        output.push(Example {
            english: runtime.english(&meaning)?,
            text: runtime.realize(&meaning)?,
            meaning,
            skill: skill.into(),
        });
    }
...
pub fn grade(package: &Package, exercise: usize, revision: &str, answer: &str) -> Result<Feedback> {
    if revision != package.revision {
        return Err(Error::Invalid(
            "This exercise belongs to a different revision. Reload the lesson".into(),
        ));
    }
    let example = package
        .examples
        .get(exercise)
        .ok_or_else(|| Error::Invalid("Unknown exercise".into()))?;
    let runtime = Runtime::new(package)?;
    let analyses = runtime.analyze(answer)?;
    let correct = analyses.contains(&example.meaning);
    Ok(Feedback {
        correct,
        answer: example.text.clone(),
        explanation: if correct {
            "Your answer has a licensed analysis matching the intended meaning.".into()
        } else {
            format!(
                "Your sentence has a different meaning. The intended meaning is: {}",
                example.english
            )
        },
    })
...
fn structure(meaning: &Meaning) -> String {
    match meaning {
        ...
        Meaning::Coordination { left, right } => {
            format!("and({},{})", structure(left), structure(right))
        }
    }
}
...
fn entity_shape(entity: &Entity) -> String {
    match entity {
        ...
        Entity::Noun {
            plural, adjective, ..
        } => format!("noun:{plural}:{}", adjective.is_some()),
    }
}
