use crate::{content, phonology::{self, Law}, random::Random};
use etyloom_core::*;
...
pub fn generate_with(
    ...
    let mut stages = vec![Stage { index: 0, name: "Ancestral".into(), grammar: grammar.clone(), events: vec![] }];
    progress("Ancestral vocabulary and paradigms are ready")?;
    for stage in 1..=recipe.history_depth {
        if canceled() { return Err(Error::Canceled); }
        let law = match stage {
        ...
        for entry in &mut entries {
            ...
            if form.base != previous.base || form.plural != previous.plural || form.past != previous.past { affected += 1; }
            entry.forms.push(form);
        }
        for form in grammar.markers.values_mut().chain(grammar.pronouns.values_mut()) {
            let changed = phonology::apply(form, law);
            if !changed.0.is_empty() { *form = changed; }
        }
        let (id, title, description) = phonology::describe(law);
        let mut events = vec![Event { id: id.into(), title: title.into(), description: description.into(), affected }];
        if stage == 4 && grammar.morphology != Morphology::Analytic {
            ...
            for entry in entries.iter_mut().filter(|e| e.category == Category::Noun) {
                ...
                let threshold = if grammar.morphology == Morphology::Mixed { 1500 } else { 6000 };
        ...
        stages.push(Stage {
            index: stage,
            name: if stage == recipe.history_depth { "Contemporary".into() } else { format!("Period {stage}") },
            grammar: grammar.clone(), events,
        });
        progress(&format!("Applied historical stage {stage} of {}", recipe.history_depth))?;
    }
    if canceled() { return Err(Error::Canceled); }
    ...
    let inventory: BTreeSet<_> = entries.iter().filter_map(|e| e.forms.last()).flat_map(|p| p.base.0.iter().copied()).collect();
...
pub fn marker<'a>(grammar: &'a Grammar, name: &str) -> Result<&'a Form> {
    grammar.markers.get(name).ok_or_else(|| Error::Package(format!("Missing marker {name}")))
}
...
fn grammar(recipe: &Recipe, sounds: &[Sound]) -> Result<Grammar> {
    ...
    let morphology = match recipe.morphology {
        Some(value) => value,
        None => rng.weighted(&[(Morphology::Analytic, 4), (Morphology::Suffixing, 4), (Morphology::Mixed, 3)])?,
    };
    let order = match recipe.order {
        Some(value) => value,
        None => rng.weighted(&[(Order::Svo, 4), (Order::Sov, 4), (Order::Vso, 2), (Order::Vos, 1), (Order::Ovs, 1), (Order::Osv, 1)])?,
    };
    ...
    if morphology != Morphology::Analytic { markers.insert("plural".into(), Form(vec![Sound::I])); }
    ...
    Ok(Grammar { morphology, order, adjective_before: rng.below(2)? == 0, case_marking: morphology != Morphology::Analytic && rng.below(4)? != 0, negation_after: rng.below(2)? == 0, markers, pronouns })
}

fn unique_marker(recipe: &Recipe, sounds: &[Sound], key: &str, used: &mut BTreeSet<String>) -> Result<Form> {
    for attempt in 0..128 {
        let mut form = phonology::word(recipe, sounds, &format!("marker/{key}/{attempt}"), true)?;
        form.0.push(Sound::L);
        if used.insert(form.text()) { return Ok(form); }
    }
    Err(Error::Budget)
}

fn ancestor(recipe: &Recipe, sounds: &[Sound], grammar: &Grammar) -> Result<Vec<Entry>> {
    content::concepts()?.into_iter().take(recipe.lexicon_size).map(|concept| {
        let base = phonology::word(recipe, sounds, &format!("lexeme/{}", concept.id), false)?;
        let paradigm = inflect(base, concept.category, grammar, 0)?;
        Ok(Entry { id: concept.id, gloss: concept.english.base.clone(), category: concept.category, transitive: concept.transitive, frequency: concept.frequency, english: concept.english, origin: Origin::Root, introduced: 0, forms: vec![paradigm] })
    }).collect()
}

pub fn inflect(base: Form, category: Category, grammar: &Grammar, stage: usize) -> Result<Paradigm> {
    let plural = if category == Category::Noun && grammar.morphology != Morphology::Analytic { Some(base.joined(marker(grammar, "plural")?)) } else { None };
    let past = if category == Category::Verb && grammar.morphology != Morphology::Analytic { Some(base.joined(marker(grammar, "past")?)) } else { None };
    Ok(Paradigm { stage, base, plural, past })
}
...
fn expand(recipe: &Recipe, grammar: &Grammar, entries: &mut Vec<Entry>) -> Result<usize> {
    ...
    let nouns: Vec<_> = entries.iter().filter(|e| e.category == Category::Noun).cloned().collect();
    let mut candidates = Vec::new();
    for modifier in &nouns {
        for head in &nouns {
            if modifier.id == head.id { continue; }
    ...
    if candidates.len() < needed { return Err(Error::Invalid("The content pack cannot supply the requested vocabulary size".into())); }
    for (_, id, modifier, head) in candidates.into_iter().take(needed) {
        let base = if grammar.adjective_before { modifier.current()?.base.joined(&head.current()?.base) } else { head.current()?.base.joined(&modifier.current()?.base) };
        let gloss = format!("{} {}", modifier.gloss, head.gloss);
        entries.push(Entry {
            id, gloss: gloss.clone(), category: Category::Noun, transitive: false, frequency: 100,
            english: English { base: gloss, plural: format!("{} {}", modifier.english.base, head.english.plural), past: String::new(), third: String::new() },
            origin: Origin::Compound { modifier: modifier.id.clone(), head: head.id.clone() },
            introduced: recipe.history_depth, forms: vec![inflect(base, Category::Noun, grammar, recipe.history_depth)?],
        });
    }
    Ok(needed)
}
...
pub fn fingerprint(package: &Package) -> Result<String> {
    ...
    Ok(blake3::hash(&serde_json::to_vec(&canonical)?).to_hex().to_string())
}
