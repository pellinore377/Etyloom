...
pub fn validate(package: &Package) -> Result<()> {
    ...
    if package.schema != SCHEMA
        || package.stages.len() != package.recipe.history_depth + 1
        || package.lexicon.len() != package.recipe.lexicon_size
    {
        return Err(Error::Package(
            "Manifest counts do not match their contents".into(),
        ));
    }
    let mut ids = BTreeSet::new();
    for entry in &package.lexicon {
        if !ids.insert(&entry.id) {
            return Err(Error::Package(format!("Duplicate entry {}", entry.id)));
        }
        if entry.introduced > package.recipe.history_depth
            || entry.forms.len() != package.recipe.history_depth + 1 - entry.introduced
        {
            return Err(Error::Package(format!(
                "Incomplete history for {}",
                entry.id
            )));
        }
        for (offset, paradigm) in entry.forms.iter().enumerate() {
            if paradigm.stage != entry.introduced + offset {
                return Err(Error::Package("Unordered historical forms".into()));
            }
    ...
    for stage in &package.stages {
        for key in [
            ...
            if generate::marker(&stage.grammar, key)?.0.is_empty() {
                return Err(Error::Package(format!("Empty grammatical marker {key}")));
            }
        }
    }
...
pub fn verify_import(package: &Package) -> Result<()> {
    validate(package)?;
    if package.revision != fingerprint(package)? {
        return Err(Error::Package(
            "Revision checksum does not match the package".into(),
        ));
    }
    Ok(())
}
